...
test.describe('Products API', () => {
  ...
  test('Get product by id', async ({ request }) => {
    const response = await request.get(`${BASE_URL}/${productId}`);
  ...
  test('Update product', async ({ request }) => {
    ...
    const response = await request.put(`${BASE_URL}/${productId}`, {
      data: {
        title: updatedTitle,
        price: 250,
  ...
  test('Delete product', async ({ request }) => {
    const response = await request.delete(`${BASE_URL}/${productId}`);
